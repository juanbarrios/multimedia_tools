...
#ifndef MKNN_PREDEFINED_DISTANCE_H_
#define MKNN_PREDEFINED_DISTANCE_H_
 ...
 * #mknn_distance_newPredefined, which requires the ID and parameters of the distance.
 *
 * The complete list of predefined distances can be listed by calling
 * #mknn_predefDistance_helpListDistances. The parameters supported by each distance
 * can be listed by calling #mknn_predefDistance_helpPrintDistance.
 *
 ...
 *
 * @return parameters to create a distance (it must be released with mknn_distanceParams_release or bound to the new distance)
 */
 ...
 * \f]
 *
 * @note This is a similarity function, therefore a search for the Farthest Neighbors is needed. See #mknn_predefDistance_CosineDistance
 ...
 * \f]
 *
 * where \f$ \cos(\vec{x},\vec{y}) \f$ is the cosine similarity between vectors \f$ \vec{x} \f$ and \f$ \vec{y} \f$ as defined in #mknn_predefDistance_CosineSimilarity.
 *
 ...
 *
 * @param num_subdistances number of subdistances to combine.
...
MknnDistanceParams *mknn_predefDistance_MultiDistance(int64_t num_subdistances,
		...
		bool free_subdistances_on_release, double *normalization_values,
